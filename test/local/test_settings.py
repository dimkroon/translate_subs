...
from resources.lib.translatesubs import settings
from resources.lib.translatesubs import addon_log


class TestSettings(unittest.TestCase):
    @patch("resources.lib.translatesubs.addon_log.set_log_handler")
    def test_change_logger(self, p_set_log):
        logger = addon_log.logger

        with patch("resources.lib.translatesubs.kodi_utils.ask_log_handler", return_value=(0, 'kodi log')):
            settings.change_logger()
            p_set_log.assert_called_with(addon_log.KodiLogHandler)

        with patch("resources.lib.translatesubs.kodi_utils.ask_log_handler", return_value=(1, 'file log')):
            settings.change_logger()
            p_set_log.assert_called_with(addon_log.CtFileHandler)

        with patch("resources.lib.translatesubs.kodi_utils.ask_log_handler", return_value=(2, 'no log')) as p_ask:
        ...
        with patch("resources.lib.translatesubs.kodi_utils.ask_log_handler", return_value=(1, 'file log')) as p_ask:
